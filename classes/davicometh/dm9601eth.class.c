...
int libExpunge(struct NepEthBase * nh)
{
    ...
    if(nh->nh_DevBase->np_Library.lib_OpenCnt == 1)
    {
        ...
        KPRINTF(5, ("libExpunge: Unloading done! dm9601eth.class expunged!\n\n"));
    } else {
...
struct NepClassEth * usbForceDeviceBinding(struct NepEthBase *nh, struct PsdDevice *pd)
{
    ...
    if((ps = OpenLibrary("poseidon.library", POSEIDON_LIB_MIN_VERSION)))
    {
        ...
        psdSafeRawDoFmt(buf, 64, "dm9601eth.class<%08lx>", ncp);
        ...
        if((tmptask = psdSpawnSubTask(buf, nEthTask, ncp)))
        {
            ...
            if(ncp->ncp_Task)
            {
                ...
                psdAddErrorMsg(RETURN_OK, (STRPTR) libname,
                               PSD_BOUND_TXT("And Goliath trembled with fear over '%s' on %s unit %ld!"),
...
void usbReleaseDeviceBinding(struct NepEthBase *nh, struct NepClassEth *ncp)
{
    ...
    if((ps = OpenLibrary("poseidon.library", POSEIDON_LIB_MIN_VERSION)))
    {
        ...
        psdAddErrorMsg(RETURN_OK, (STRPTR) libname,
                       PSD_RELEASED_TXT("But finally Goliath crushed '%s'."),
                       devname);
        /*psdFreeVec(ncp);*/
        CloseLibrary(ps);
    }
...
LONG (usbGetAttrsA)(ULONG type asm("d0"), APTR usbstruct asm("a0"), struct TagItem * tags asm("a1"), struct NepEthBase * nh asm("a6"))
{
    ...
    switch(type)
    {
        case UGA_CLASS:
             ...
             {
                 *((STRPTR *) ti->ti_Data) = "Ethernet SANA driver for Davicom chipsets via dm9601eth.device";
                 count++;
             }
...
BOOL nLoadClassConfig(struct NepEthBase *nh)
{
    ...
    cdc = ncp->ncp_CDC;
    cdc->cdc_ChunkID = AROS_LONG2BE(MAKE_ID('D','M','9','6'));
...
void nEthTask()
{
    ...
    if((ncp = nAllocEth()))
    {
        ...
        /*if(nReadEEPROMMAC(ncp, ncp->ncp_ROMAddress))
        {
            ...
            nWriteDMRegs(ncp, ncp->ncp_MacAddress, ETHER_ADDR_SIZE, DMREG_ETH_ID);
        } else {*/
            /*psdAddErrorMsg(RETURN_WARN, (STRPTR) libname,
                           "Couldn't read EEPROM for MAC Address, using preset one.");*/
            nReadDMRegs(ncp, ncp->ncp_ROMAddress, ETHER_ADDR_SIZE, DMREG_ETH_ID);
            if(!(ncp->ncp_StateFlags & DDF_CONFIGURED))
            {
                // don't overwrite previously configured MAC Address
                CopyMem(ncp->ncp_ROMAddress, ncp->ncp_MacAddress, ETHER_ADDR_SIZE);
        ...
        do
        {
            ...
            while((ioreq = (struct IOSana2Req *) GetMsg(&ncp->ncp_Unit.unit_MsgPort)))
            {
                ...
                switch(ioreq->ios2_Req.io_Command)
                {
                    ...
                    {
                        ULONG rxcr = RX_ENABLE|RX_ALL_MULTICAST|RX_DISCARD_LONG|RX_DISCARD_CRC;
                        ...
                        {
                            rxcr &= ~RX_ALL_MULTICAST;
                            nWriteDMRegs(ncp, ncp->ncp_MulticastArray, 8, DMREG_MCAST_ADDR);
                        }
                        nWriteDMReg(ncp, DMREG_RX_CTRL, rxcr);
                    ...
                    case S2_OFFLINE:
                        ioerr = nWriteDMReg(ncp, DMREG_RX_CTRL, 0x00);

                        nWritePhyWord(ncp, MII_BMCR, BMCR_PDOWN);
                        ReplyMsg((struct Message *) ioreq);
                        break;
...
LONG nReadDMReg(struct NepClassEth *ncp, ULONG reg)
{
    ...
    psdPipeSetup(ncp->ncp_EP0Pipe, URTF_IN|URTF_DEVICE|URTF_VENDOR, UDMR_GET_REGS, 0, reg);
    ioerr = psdDoPipe(ncp->ncp_EP0Pipe, &result, 1);
    if(ioerr)
    {
        psdAddErrorMsg(RETURN_WARN, (STRPTR) libname,
                       "Reading DM Reg %ld failed: %s (%ld)",
                       reg,
                       psdNumToStr(NTS_IOERR, ioerr, "unknown"), ioerr);
        return(-1);
    }
    KPRINTF(1, ("DM Reg(%ld)=%02lx (%ld)\n", reg, result, result));
    return(result);
}
/* \\\ */
...
LONG nReadDMRegs(struct NepClassEth *ncp, UBYTE *data, ULONG len, ULONG offset)
{
    ...
    psdPipeSetup(ncp->ncp_EP0Pipe, URTF_IN|URTF_DEVICE|URTF_VENDOR, UDMR_GET_REGS, 0, offset);
    ioerr = psdDoPipe(ncp->ncp_EP0Pipe, data, len);
    if(ioerr)
    {
        psdAddErrorMsg(RETURN_WARN, (STRPTR) libname,
                       "Reading DM Regs %ld-%ld failed: %s (%ld)",
                       offset, offset+len-1,
                       psdNumToStr(NTS_IOERR, ioerr, "unknown"), ioerr);
    }
    return(ioerr);
...
LONG nWriteDMReg(struct NepClassEth *ncp, ULONG reg, ULONG value)
{
    ...
    psdPipeSetup(ncp->ncp_EP0Pipe, URTF_OUT|URTF_DEVICE|URTF_VENDOR, UDMR_SET_REG, value, reg);
    ioerr = psdDoPipe(ncp->ncp_EP0Pipe, NULL, 0);
    if(ioerr)
    {
        psdAddErrorMsg(RETURN_WARN, (STRPTR) libname,
                       "Writing DM Reg %ld with 0x%02lx failed: %s (%ld)",
                       reg, value,
                       psdNumToStr(NTS_IOERR, ioerr, "unknown"), ioerr);
    }
    return(ioerr);
...
LONG nWriteDMRegs(struct NepClassEth *ncp, UBYTE *data, ULONG len, ULONG offset)
{
    ...
    psdPipeSetup(ncp->ncp_EP0Pipe, URTF_OUT|URTF_DEVICE|URTF_VENDOR, UDMR_SET_REGS, 0, offset);
    ioerr = psdDoPipe(ncp->ncp_EP0Pipe, data, len);
    if(ioerr)
    {
        psdAddErrorMsg(RETURN_WARN, (STRPTR) libname,
                       "Writing DM Regs %ld-%ld failed: %s (%ld)",
                       offset, offset+len-1,
                       psdNumToStr(NTS_IOERR, ioerr, "unknown"), ioerr);
    }
    return(ioerr);
...
LONG nReadPhyWord(struct NepClassEth *ncp, ULONG phyreg)
{
    ...
    ioerr = nWriteDMReg(ncp, DMREG_PHY_ADDR, phyreg|0x40);
    if(ioerr)
    {
        return(-1);
    }
    ioerr = nWriteDMReg(ncp, DMREG_PHY_CTRL, PHY_READ|PHY_PHYSEL);
    if(ioerr)
    {
        return(-1);
    }
    ...
    ioerr = nWriteDMReg(ncp, DMREG_PHY_CTRL, 0x00);
    if(ioerr)
    {
        return(-1);
    }
    ...
    ioerr = nReadDMRegs(ncp, phyword, 2, DMREG_PHY_DATA_LOW);
    if(ioerr)
    {
        return(-1);
    }
...
BOOL nWritePhyWord(struct NepClassEth *ncp, ULONG phyreg, ULONG value)
{
    ...
    ioerr = nWriteDMRegs(ncp, val, 2, DMREG_PHY_DATA_LOW);
    if(ioerr)
    {
        return(FALSE);
    }
    ...
    ioerr = nWriteDMReg(ncp, DMREG_PHY_CTRL, PHY_PHYSEL|PHY_WRITE);
    if(ioerr)
    {
        return(FALSE);
    }
    ...
    if(!timeout)
    {
        KPRINTF(20, ("Timeout waiting for PHY to respond (write reg %ld=%04lx)!\n", phyreg, value));
        return(FALSE);
    }
    return(TRUE);
}
...
BOOL nInitDavicom(struct NepClassEth *ncp)
{
    ...
    ioerr = nWriteDMReg(ncp, DMREG_NET_CTRL, 0x01);
    if(ioerr)
    {
        return(FALSE);
    }
    ...
    ioerr = nWriteDMReg(ncp, DMREG_GPR_CTRL, 0x01);
    if(ioerr)
    {
        return(FALSE);
    }
    ioerr = nWriteDMReg(ncp, DMREG_GPR_DATA, 0x00);
    if(ioerr)
    {
        return(FALSE);
    }
    ...
    psdAddErrorMsg(RETURN_WARN, (STRPTR) libname, "Couldn't identify PHY!");
    return(FALSE);
}
/* \\\ */
...
void nSetOnline(struct NepClassEth *ncp)
{
    ...
    nWritePhyWord(ncp, MII_BMCR, bmcr);
    while(--timeout)
    {
        psdDelayMS(50);
        data = nReadPhyWord(ncp, MII_BMSR);
    ...
    rxcr = RX_ENABLE|RX_ALL_MULTICAST|RX_DISCARD_LONG|RX_DISCARD_CRC;
    nWriteDMRegs(ncp, ncp->ncp_MulticastArray, 8, DMREG_MCAST_ADDR);
    if(ncp->ncp_OpenFlags & SANA2OPF_PROM)
    {
        rxcr |= RX_PROMISCUOUS;
    ...
    data = nReadPhyWord(ncp, MII_TPISTATUS);
    nWritePhyWord(ncp, MII_TPISTATUS, data|4);
...
BOOL nWritePacket(struct NepClassEth *ncp, struct IOSana2Req *ioreq)
{
    ...
    if(!((writelen+2) & (ncp->ncp_EPOutMaxPktSize-1)))
    {
...
UWORD nReadIOReq(struct NepClassEth *ncp, struct EtherPacketHeader *eph, UWORD datasize, struct IOSana2Req *ioreq, UWORD flags)
{
    ...
    {
        /* Address == Broadcast? */
        const UBYTE bcast[ETHER_ADDR_SIZE] = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff };
