/* dev.c - lan78xx.class
 *
 * Based on usbcdceth.device by Chris Hodges.
 */
...
DEVBASETYPEPTR (devOpen)(struct IOSana2Req * ioreq asm("a1"), ULONG unit asm("d0"), ULONG flags asm("d1"), struct NepEthDevBase * base asm("a6"))
{
    ...
    KPRINTF(10,
            ("devOpen ioreq: 0x%08lx unit: %ld flags: 0x%08lx base: 0x%08lx\n", (void *)ioreq, unit, flags, (void *)base));
    KPRINTF(1, ("devOpen: request unit=%lu flags=0x%08lx base=0x%08lx clsbase=0x%08lx\n", unit, flags, base,
                base ? base->np_ClsBase : NULL));
    ...
    if (ioreq->ios2_Req.io_Message.mn_Length && (ioreq->ios2_Req.io_Message.mn_Length < sizeof(struct IOSana2Req))) {
        ...
        KPRINTF(1, ("devOpen: scanning available units for %s\n", DEVNAME));
        while (ncp->ncp_Unit.unit_MsgPort.mp_Node.ln_Succ) {
            KPRINTF(1, ("devOpen: candidate node=0x%08lx unit=%lu task=0x%08lx openflags=0x%08lx devid='%s'\n", ncp,
                        ncp->ncp_UnitNo, ncp->ncp_Task, ncp->ncp_OpenFlags, ncp->ncp_DevIDString));
            if (ncp->ncp_UnitNo == unit) {
                if ((ncp->ncp_OpenFlags & SANA2OPF_MINE) ||
                    ((flags & SANA2OPF_MINE) && ncp->ncp_Unit.unit_OpenCnt > 0)) {
                    ioreq->ios2_Req.io_Error = IOERR_UNITBUSY;
                    KPRINTF(1, ("devOpen: matched unit %lu but it is busy\n", unit));
                } else {
                    ioreq->ios2_Req.io_Unit = (struct Unit *)ncp;
                    KPRINTF(1, ("devOpen: matched unit %lu with node=0x%08lx task=0x%08lx\n", unit, ncp, ncp->ncp_Task));
                }
                break;
            }
        ...
        if (!ioreq->ios2_Req.io_Unit) {
            /* Preserve IOERR_UNITBUSY from the match-but-busy path above;
             * only fall through to OPENFAIL if no candidate matched at all. */
            if (ioreq->ios2_Req.io_Error != IOERR_UNITBUSY)
                ioreq->ios2_Req.io_Error = IOERR_OPENFAIL;
            KPRINTF(20, ("devOpen: could not open unit!\n"));
            KPRINTF(1, ("devOpen: no matching unit %lu found for %s\n", unit, DEVNAME));
        } else {
            ...
            if (taglist) {
                ...
                if (bufman) {
                    ...
                    KPRINTF(20, ("Open_Unit: failed to allocate bufman!\n"));
                    ioreq->ios2_Req.io_Error = IOERR_OPENFAIL;
...
BPTR (devClose)(struct IOSana2Req * ioreq asm("a1"), struct NepEthDevBase * base asm("a6"))
{
    ...
    if (bufman) {
        ...
        Disable();
        /* Drain any reads/writes still queued against this opener so we
         * don't leave dangling ioreqs pointing at memory we're about to
         * free. */
...
static const struct Sana2DeviceQuery DeviceQueryBlock[] = {{sizeof(struct Sana2DeviceQuery),
                                                            sizeof(struct Sana2DeviceQuery), 0, 0, ETHER_ADDR_SIZE * 8,
                                                            ETHERPKT_SIZE, 10000000, S2WireType_Ethernet}};
...
WORD cmdDeviceQuery(struct NepClassEth *ncp, struct IOSana2Req *ioreq)
{
    ...
    CopyMem(&((struct Sana2DeviceQuery *)DeviceQueryBlock)->SizeSupplied, &query->SizeSupplied,
            sizeof(struct Sana2DeviceQuery) - sizeof(ULONG));
...
WORD cmdConfigInterface(struct NepClassEth *ncp, struct IOSana2Req *ioreq)
{
    ...
    if (ncp->ncp_StateFlags & DDF_CONFIGURED) {
        Enable();
        /* Copy the address (reverse direction) */
        for (cnt = 0; cnt < ETHER_ADDR_SIZE; cnt++) {
            ioreq->ios2_SrcAddr[cnt] = ncp->ncp_MacAddress[cnt];
        }
...
{
    KPRINTF(1, ("S2_ADDMULTICASTADDRESS 0x%08lx%04lx\n", ((ULONG *)ioreq->ios2_SrcAddr)[0],
                ((UWORD *)ioreq->ios2_SrcAddr)[2]));
...
{
    KPRINTF(1,
            ("S2_ADDMULTICASTADDRESSES 0x%08lx%04lx - 0x%08lx%04lx\n", ((ULONG *)ioreq->ios2_SrcAddr)[0],
             ((UWORD *)ioreq->ios2_SrcAddr)[2], ((ULONG *)ioreq->ios2_DstAddr)[0], ((UWORD *)ioreq->ios2_DstAddr)[2]));
...
{
    KPRINTF(1,
            ("S2_DELMULTICASTADDRESSES 0x%08lx%04lx - 0x%08lx%04lx\n", ((ULONG *)ioreq->ios2_SrcAddr)[0],
             ((UWORD *)ioreq->ios2_SrcAddr)[2], ((ULONG *)ioreq->ios2_DstAddr)[0], ((UWORD *)ioreq->ios2_DstAddr)[2]));
...
WORD AddMCastRange(struct NepClassEth *ncp, struct IOSana2Req *ioreq, UBYTE *lower, UBYTE *upper)
{
    ...
    /* Pre-allocate outside Disable() since AllocVec() may block */
    {
        struct MulticastAddressRange *newmar;
        newmar = AllocVec(sizeof(struct MulticastAddressRange), MEMF_CLEAR | MEMF_PUBLIC);
        if (!newmar) {
        ...
        while (mar->mar_Node.ln_Succ) {
            if (mcmp(lower, mar->mar_LowerAddr) && mcmp(upper, mar->mar_UpperAddr)) {
                break;
            }
        ...
        /* Existing range found, free unused pre-allocated node */
        FreeVec(newmar);
    }
...
ULONG ether_crc(LONG length, UBYTE *data)
{
    register LONG crc;
...
{
    KPRINTF(1, ("S2_MULTICAST ioreq: 0x%08lx to: 0x%08lx%04lx\n", (void *)ioreq, ((ULONG *)ioreq->ios2_DstAddr)[0],
                ((UWORD *)ioreq->ios2_DstAddr)[2]));
...
WORD cmdTrackType(struct NepClassEth *ncp, struct IOSana2Req *ioreq)
{
    ...
    /* Find matching node */
    {
        struct PacketTypeStats *existing;
        existing = (struct PacketTypeStats *)ncp->ncp_TrackList.lh_Head;
        while (existing->pts_Node.ln_Succ) {
            /* Our packet type? */
            if (existing->pts_PacketType == ioreq->ios2_PacketType) {
                Enable();
                FreeVec(pts);
                return deverror(S2ERR_BAD_STATE, S2WERR_ALREADY_TRACKED);
            }
            existing = (struct PacketTypeStats *)existing->pts_Node.ln_Succ;
        }
    }
...
#define KNOWN_EVENTS                                                                                                   \
    (S2EVENT_ERROR | S2EVENT_TX | S2EVENT_RX | S2EVENT_ONLINE | S2EVENT_OFFLINE | S2EVENT_BUFF | S2EVENT_HARDWARE)

WORD cmdOnEvent(struct NepClassEth *ncp, struct IOSana2Req *ioreq)
{
    KPRINTF(1, ("S2_ONEVENT ioreq: 0x%08lx mask: 0x%08lx\n", (void *)ioreq, ioreq->ios2_WireError));
...
static const UWORD NSDSupported[] = {CMD_FLUSH,
                                     CMD_READ,
                                     CMD_WRITE,
                                     S2_DEVICEQUERY,
...
WORD cmdNSDeviceQuery(struct NepClassEth *ncp, struct IOStdReq *ioreq)
{
    ...
    /* NULL ptr?
       ...
       Valid request?
    */
    if ((!query) || (ioreq->io_Length < sizeof(struct my_NSDeviceQueryResult)) || (query->DevQueryFormat != 0) ||
    ...
    ioreq->io_Actual = query->SizeAvailable = sizeof(struct my_NSDeviceQueryResult);
...
void AbortList(struct NepClassEth *ncp, struct List *list, struct BufMan *bufman, WORD error)
{
    ...
    if (bufman) {
        for (ioreq = (struct IOSana2Req *)list->lh_Head;
             (nextioreq = (struct IOSana2Req *)ioreq->ios2_Req.io_Message.mn_Node.ln_Succ); ioreq = nextioreq) {
