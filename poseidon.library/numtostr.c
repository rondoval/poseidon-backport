...
{
    { AUDIO_CLASSCODE,     "Audio" },
    { CDCCTRL_CLASSCODE,   "CDC control" },
    { CDCDATA_CLASSCODE,   "CDC data" },
    { HID_CLASSCODE,       "HID" },
    { PHYSICAL_CLASSCODE,  "Physical" },
    { STILLIMG_CLASSCODE,  "Still Images" },
    { PRINTER_CLASSCODE,   "Printer" },
    { MASSSTORE_CLASSCODE, "MassStorage" },
    { HUB_CLASSCODE,       "Hub" },
    { SMARTCARD_CLASSCODE, "Chip-/Smartcard" },
    { SECURITY_CLASSCODE,  "Content security" },
    { VIDEO_CLASSCODE,     "Video Device" },
    { MISC_CLASSCODE,      "Miscellaneous Device" },
...
/* 010000CS Class
   0300SCCS Subclass-Class
   07PRSCCS Proto-Subclass-Class
   05PR00CS Proto-Class */

const struct PsdULStringMap usbcomboclasscodestr[] =
{
    ...
    { (3<<24)|(CDC_ACM_SUBCLASS     <<8)|CDCCTRL_CLASSCODE  , "Abstract Control (Modem)" },
    { (3<<24)|(CDC_TCM_SUBCLASS     <<8)|CDCCTRL_CLASSCODE  , "Telephone Control" },
    { (3<<24)|(CDC_MCCM_SUBCLASS    <<8)|CDCCTRL_CLASSCODE  , "Multi-Channel Control" },
    { (3<<24)|(CDC_CAPICM_SUBCLASS  <<8)|CDCCTRL_CLASSCODE  , "CAPI Control (ISDN)" },
    { (3<<24)|(CDC_ETHCM_SUBCLASS   <<8)|CDCCTRL_CLASSCODE  , "Ethernet Networking Control" },
    { (3<<24)|(CDC_ATMCM_SUBCLASS   <<8)|CDCCTRL_CLASSCODE  , "ATM Networking Control" },
    { (3<<24)|(CDC_WHCM_SUBCLASS    <<8)|CDCCTRL_CLASSCODE  , "Wireless Handset Control" },
    ...
    { (7<<24)|(HID_PROTO_KEYBOARD<<16)|(HID_BOOT_SUBCLASS<<8)|HID_CLASSCODE, "HID (Keyboard)" },
    { (7<<24)|(HID_PROTO_MOUSE   <<16)|(HID_BOOT_SUBCLASS<<8)|HID_CLASSCODE, "HID (Mouse)" },
    { (1<<24)                           |HID_CLASSCODE      , "HID" },

    { (1<<24)                           |PHYSICAL_CLASSCODE , "Physical" },

    { (7<<24)|(1<<16)|(1<<8)            |STILLIMG_CLASSCODE , "PTP Camera" },
    { (3<<24)|(1<<8)                    |STILLIMG_CLASSCODE , "Still Images" },
    { (1<<24)                           |STILLIMG_CLASSCODE , "Imaging" },

    { (7<<24)|(PRT_PROTO_UNIDIR  <<16)|(1<<8)|PRINTER_CLASSCODE, "Printer (Unidirectional)" },
    { (7<<24)|(PRT_PROTO_BIDIR   <<16)|(1<<8)|PRINTER_CLASSCODE, "Printer (Bidirectional)" },
    { (7<<24)|(PRT_PROTO_IEEE1284<<16)|(1<<8)|PRINTER_CLASSCODE, "Printer (IEEE1284.4)" },
    { (1<<24)                           |PRINTER_CLASSCODE  , "Printer" },

    { (7<<24)|(MS_PROTO_CBI <<16)|(MS_UFI_SUBCLASS     <<8)|MASSSTORE_CLASSCODE, "Floppy" },
    { (7<<24)|(MS_PROTO_CBI <<16)|(MS_FDDATAPI_SUBCLASS<<8)|MASSSTORE_CLASSCODE, "Floppy" },
    { (7<<24)|(MS_PROTO_CB  <<16)|(MS_UFI_SUBCLASS     <<8)|MASSSTORE_CLASSCODE, "Floppy" },
    ...
    { (3<<24)|(2<<8)                    |VIDEO_CLASSCODE    , "Video Streaming" },
    { (3<<24)|(3<<8)                    |VIDEO_CLASSCODE    , "Video Interface Collection" },
    ...
    { (3<<24)|(1<<8)                    |FWUPGRADE_CLASSCODE, "Firmware upgrade" },
    { (3<<24)|(2<<8)                    |FWUPGRADE_CLASSCODE, "IrDA Bridge" },
...
/* 000000DT DescriptorType
   0000CSDT Class-DescriptorType
   00SCCSDT Subclass-Class-DescriptorType
   ST00CSDT SubType-Class-DescriptorType
   STSCCSDT SubType-Subclass-Class-DescriptorType */

const struct PsdULStringMap usbdesctypestr[] =
{
    { UDT_DEVICE,                "Device" },
    { UDT_CONFIGURATION,         "Configuration" },
    { UDT_STRING,                "String" },
    { UDT_INTERFACE,             "Interface" },
    { UDT_ENDPOINT,              "Endpoint" },
    { UDT_DEVICE_QUALIFIER,      "Device Qualifier" },
    { UDT_OTHERSPEED_QUALIFIER,  "Otherspeed Qualifier" },
    { UDT_INTERFACE_POWER,       "Interface Power" },
    { UDT_OTG,                   "On-The-Go" },
    { UDT_DEBUG,                 "Debug" },
    { UDT_INTERFACE_ASSOCIATION, "Interface Association" },
    { UDT_SECURITY,              "WUSB Security" },
    { UDT_BOS,                   "WUSB Binary Device Object" },
    { UDT_DEVICE_CAPABILITY,     "WUSB Device Capability" },
    { UDT_ENCRYPTION_TYPE,       "WUSB Encryption Type" },
    { UDT_WIRELESS_EP_COMP,      "WUSB Wireless Endpoint Companion" },

    { UDT_CS_DEVICE,             "Class-specific Device" },
    { UDT_CS_CONFIGURATION,      "Class-specific Configuration" },
    { UDT_CS_STRING,             "Class-specific String" },
    { UDT_CS_INTERFACE,          "Class-specific Interface" },
    { UDT_CS_ENDPOINT,           "Class-specific Endpoint" },

    { (HUB_CLASSCODE<<8)|UDT_HUB,      "Hub" },

    { (HID_CLASSCODE<<8)|UDT_HID,      "HID" },
    { (HID_CLASSCODE<<8)|UDT_REPORT,   "HID Report" },
    { (HID_CLASSCODE<<8)|UDT_PHYSICAL, "HID Physical" },

    { (FWUPGRADE_CLASSCODE<<8)|UDT_DFU, "Firmware Upgrade Info" },

    { (MISC_COMMON_SUBCLASS<<16)|(MISC_CLASSCODE<<8)|UDT_WIREADAPTER_CLASS, "WUSB Wire Adapter" },

    { (UDST_AUDIO_CTRL_HEADER         <<24)|(AUDIO_CTRL_SUBCLASS<<16)|(AUDIO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Audio Control Header" },
    { (UDST_AUDIO_CTRL_INPUT_TERMINAL <<24)|(AUDIO_CTRL_SUBCLASS<<16)|(AUDIO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Audio Input Terminal" },
    { (UDST_AUDIO_CTRL_OUTPUT_TERMINAL<<24)|(AUDIO_CTRL_SUBCLASS<<16)|(AUDIO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Audio Output Terminal" },
    { (UDST_AUDIO_CTRL_MIXER_UNIT     <<24)|(AUDIO_CTRL_SUBCLASS<<16)|(AUDIO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Audio Mixer Unit" },
    { (UDST_AUDIO_CTRL_SELECTOR_UNIT  <<24)|(AUDIO_CTRL_SUBCLASS<<16)|(AUDIO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Audio Selector Unit" },
    { (UDST_AUDIO_CTRL_FEATURE_UNIT   <<24)|(AUDIO_CTRL_SUBCLASS<<16)|(AUDIO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Audio Feature Unit" },
    { (UDST_AUDIO_CTRL_PROCESSING_UNIT<<24)|(AUDIO_CTRL_SUBCLASS<<16)|(AUDIO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Audio Processing Unit" },
    { (UDST_AUDIO_CTRL_EXTENSION_UNIT <<24)|(AUDIO_CTRL_SUBCLASS<<16)|(AUDIO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Audio Extension Unit" },

    { (UDST_AUDIO_STREAM_GENERAL     <<24)|(AUDIO_STREAM_SUBCLASS<<16)|(AUDIO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Audio Stream General" },
    { (UDST_AUDIO_STREAM_FMT_TYPE    <<24)|(AUDIO_STREAM_SUBCLASS<<16)|(AUDIO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Audio Stream Format Type" },
    { (UDST_AUDIO_STREAM_FMT_SPECIFIC<<24)|(AUDIO_STREAM_SUBCLASS<<16)|(AUDIO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Audio Stream Format Specific" },

    { (UDST_AUDIO_MIDI_HEADER  <<24)|(AUDIO_MIDI_SUBCLASS<<16)|(AUDIO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Midi Header" },
    { (UDST_AUDIO_MIDI_IN_JACK <<24)|(AUDIO_MIDI_SUBCLASS<<16)|(AUDIO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Midi IN Jack" },
    { (UDST_AUDIO_MIDI_OUT_JACK<<24)|(AUDIO_MIDI_SUBCLASS<<16)|(AUDIO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Midi OUT Jack" },
    { (UDST_AUDIO_MIDI_ELEMENT <<24)|(AUDIO_MIDI_SUBCLASS<<16)|(AUDIO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Midi Element" },

    { (UDST_AUDIO_EP_GENERAL   <<24)|(AUDIO_CLASSCODE<<8)|UDT_CS_ENDPOINT, "Audio Generic Endpoint" },

    { (UDST_CDC_HEADER      <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "CDC Header" },
    { (UDST_CDC_CALLMGM     <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "Call Management" },
    { (UDST_CDC_ACM         <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "Abstract Control Management" },
    { (UDST_CDC_DIRECTLINE  <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "Direct Line Management" },
    { (UDST_CDC_PHONERING   <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "Telephone Ringer" },
    { (UDST_CDC_PHONECALL   <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "Telephone Call and Line State Reporting Capabilities" },
    { (UDST_CDC_UNION       <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "Union" },
    { (UDST_CDC_COUNTRY     <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "Country Selection" },
    { (UDST_CDC_PHONEOP     <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "Telephone Operational Modes" },
    { (UDST_CDC_TERMINAL    <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "USB Terminal" },
    { (UDST_CDC_NETWORK     <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "Network Channel Terminal" },
    { (UDST_CDC_PROTOCOL    <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "Protocol Unit" },
    { (UDST_CDC_EXTENSION   <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "Extension Unit" },
    { (UDST_CDC_MCM         <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "Multi-Channel Management" },
    { (UDST_CDC_CAPICTRL    <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "CAPI Control Management" },
    { (UDST_CDC_ETHERNET    <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "Ethernet Networking" },
    { (UDST_CDC_ATM         <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "ATM Networking" },
    { (UDST_CDC_WMC         <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "Wireless Handset Control Model" },
    { (UDST_CDC_MDLM        <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "Mobile Direct Line Model" },
    { (UDST_CDC_MDLMDETAIL  <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "MDLM Detail" },
    { (UDST_CDC_DEVMANGM    <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "Device Management Model" },
    { (UDST_CDC_OBEX        <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "OBEX" },
    { (UDST_CDC_CMDSET      <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "Command Set" },
    { (UDST_CDC_CMDSETDETAIL<<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "Command Set Detail" },
    { (UDST_CDC_TCM         <<24)|(CDCCTRL_CLASSCODE<<8)|UDT_CS_INTERFACE, "Telephone Control Model" },

    { (SMARTCARD_CLASSCODE<<8)|UDT_CCID, "Smartcard Functional Descriptor" },

    { (UDST_VIDEO_CTRL_HEADER         <<24)|(VIDEO_CTRL_SUBCLASS<<16)|(VIDEO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Video Control Header" },
    { (UDST_VIDEO_CTRL_INPUT_TERMINAL <<24)|(VIDEO_CTRL_SUBCLASS<<16)|(VIDEO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Video Input Terminal" },
    { (UDST_VIDEO_CTRL_OUTPUT_TERMINAL<<24)|(VIDEO_CTRL_SUBCLASS<<16)|(VIDEO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Video Output Terminal" },
    { (UDST_VIDEO_CTRL_SELECTOR_UNIT  <<24)|(VIDEO_CTRL_SUBCLASS<<16)|(VIDEO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Video Selector Unit" },
    { (UDST_VIDEO_CTRL_PROCESSING_UNIT<<24)|(VIDEO_CTRL_SUBCLASS<<16)|(VIDEO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Video Processing Unit" },
    { (UDST_VIDEO_CTRL_EXTENSION_UNIT <<24)|(VIDEO_CTRL_SUBCLASS<<16)|(VIDEO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Video Extension Unit" },

    { (UDST_VIDEO_STREAM_INPUT_HEADER       <<24)|(VIDEO_STREAM_SUBCLASS<<16)|(VIDEO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Video Stream Input Header" },
    { (UDST_VIDEO_STREAM_OUTPUT_HEADER      <<24)|(VIDEO_STREAM_SUBCLASS<<16)|(VIDEO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Video Stream Output Header" },
    { (UDST_VIDEO_STREAM_STILL_IMAGE_FRAME  <<24)|(VIDEO_STREAM_SUBCLASS<<16)|(VIDEO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Video Stream Still Image Frame" },
    { (UDST_VIDEO_STREAM_FORMAT_UNCOMPRESSED<<24)|(VIDEO_STREAM_SUBCLASS<<16)|(VIDEO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Video Stream Format Uncompressed" },
    { (UDST_VIDEO_STREAM_FRAME_UNCOMPRESSED <<24)|(VIDEO_STREAM_SUBCLASS<<16)|(VIDEO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Video Stream Frame Uncompressed" },
    { (UDST_VIDEO_STREAM_FORMAT_MJPEG       <<24)|(VIDEO_STREAM_SUBCLASS<<16)|(VIDEO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Video Stream Format MJPEG" },
    { (UDST_VIDEO_STREAM_FRAME_MJPEG        <<24)|(VIDEO_STREAM_SUBCLASS<<16)|(VIDEO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Video Stream Frame MJPEG" },
    { (UDST_VIDEO_STREAM_FORMAT_MPEG2TS     <<24)|(VIDEO_STREAM_SUBCLASS<<16)|(VIDEO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Video Stream Format MPEG2-TS" },
    { (UDST_VIDEO_STREAM_FORMAT_DV          <<24)|(VIDEO_STREAM_SUBCLASS<<16)|(VIDEO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Video Stream Format DV" },
    { (UDST_VIDEO_STREAM_COLORFORMAT        <<24)|(VIDEO_STREAM_SUBCLASS<<16)|(VIDEO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Video Stream ColorFormat" },
    { (UDST_VIDEO_STREAM_FORMAT_FRAME_BASED <<24)|(VIDEO_STREAM_SUBCLASS<<16)|(VIDEO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Video Stream Format Frame-based" },
    { (UDST_VIDEO_STREAM_FRAME_FRAME_BASED  <<24)|(VIDEO_STREAM_SUBCLASS<<16)|(VIDEO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Video Stream Frame Frame-based" },
    { (UDST_VIDEO_STREAM_FORMAT_STREAM_BASED<<24)|(VIDEO_STREAM_SUBCLASS<<16)|(VIDEO_CLASSCODE<<8)|UDT_CS_INTERFACE, "Video Stream Format Stream-based" },

    { (UDST_VIDEO_EP_GENERAL  <<24)|(VIDEO_CLASSCODE<<8)|UDT_CS_ENDPOINT, "Video Generic Endpoint" },
    { (UDST_VIDEO_EP_ENDPOINT <<24)|(VIDEO_CLASSCODE<<8)|UDT_CS_ENDPOINT, "Video Endpoint Endpoint" },
    { (UDST_VIDEO_EP_INTERRUPT<<24)|(VIDEO_CLASSCODE<<8)|UDT_CS_ENDPOINT, "Video Interrupt Endpoint" },
...
/* /// "usbhwioerrstr[]" */
const struct PsdWStringMap usbhwioerrstr[] =
{
    { UHIOERR_USBOFFLINE,  "usb bus offline" },
    { UHIOERR_NAK,         "nak received" },
    { UHIOERR_HOSTERROR,   "host error" },
    { UHIOERR_STALL,       "endpoint stall" },
    { UHIOERR_PKTTOOLARGE, "packet too large" },
    { UHIOERR_TIMEOUT,     "response timeout" },
    { UHIOERR_OVERFLOW,    "packet overflow" },
    { UHIOERR_CRCERROR,    "packet crc error" },
    { UHIOERR_RUNTPACKET,  "short packet received" },
    { UHIOERR_NAKTIMEOUT,  "nak timeout" },
...
/* /// "usblangids[]" */
const struct PsdUWStringMap usblangids[] =
{
...
/* /// "usbvendorids[]" */
const struct PsdUWStringMap usbvendorids[] =
{
    /* new OpenBSD: usbdevs,v 1.77 2004/02/27, usb.ids,v 1.147 2004/04/30 16:18:32 vojtech */
    { 0x0386, "LTS" },
    ...
    { 0x07fa, "Draytek" },
    { 0x07fc, "Siemens Business Services" },
    ...
    { 0x0a72, "Sanwa Denshi" },
    { 0x0a79, "Geocast Network Systems" },
    { 0x0a7d, "NSTL, Inc." },
    ...
    { 0x0b7e, "Kurusugawa Electronics, Inc." },
    ...
    { 0x1183, "Compaq Computer Corp. (Digital Dream?)" },
