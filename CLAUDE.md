...
A backport of the **AROS Poseidon USB stack to AmigaOS 3.2** (m68k, bebbo gcc, NDK 3.2, cmake).
...
`scripts/docker-build.sh`, which owns the docker invocation; CI runs the same wrapper.
...
- **RawDoFmt rules** (OS3.2 exec): no `%p` (use `0x%08lx`); always `l`-size (`%ld`/`%lu`/`%lx`) —
  bare `%d` reads 16 bits but bebbo varargs push 32.
...
- MUI 5 SDK's broken `__inline MUI_NewObject` under bebbo `-O2` is worked around by a
  force-included va_list replacement — don't include the SDK's inline version directly.
